--@name Projectiles, damage and ETC v3 - Safer Net Payload and Kill Credit
--@author AstricUnion
--@shared
...
if SERVER then
    ...
    function Laser:start()
        ...
        net.writeTable({
            ...
            damage_diameter = self.damage_diameter,
            filter = self.filter
        })
    ...
    function Laser:think(callback)
        ...
        local attacker = isValid(self.owner) and self.owner or nil
        for _, ent in ipairs(ents) do
            ...
            if damagePermitted then
                ent:applyDamage(self.damage, attacker)
            end
        end
    end
